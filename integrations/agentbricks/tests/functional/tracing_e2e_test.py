"""Functional e2e for `agentbricks tracing`'s local dev store, read back by the REAL CLI.

Writes a trace to the sqlite-backed MLflow server that `agentbricks dev` starts, then reads it with the
actual `agentbricks tracing list` / `get`, which spin up a short-lived server and read over REST (never
...
def local_trace_store(tmp_path: pathlib.Path) -> pathlib.Path:
    ...
    Uses agentbricks's own ``start_local_tracing_server`` (the real ``uvx mlflow`` server `agentbricks dev` runs),
...
def _agentbricks_json(*args: str, project: pathlib.Path, home: pathlib.Path):
    """Run the real `agentbricks` CLI with `--output json`, isolated HOME, and return the parsed stdout.

    Inherits PATH so the read command can find `uvx`, but points HOME at an empty dir (no agentbricks login /
...
def test_tracing_list_and_get_read_the_local_dev_store(
    ...
    listed = _agentbricks_json("tracing", "list", project=local_trace_store, home=home)
    assert listed, "`agentbricks tracing list` returned no local traces"
    trace_id = listed[0]["trace_id"]
    assert isinstance(trace_id, str) and trace_id.startswith("tr-")

    got = _agentbricks_json("tracing", "get", trace_id, project=local_trace_store, home=home)
