...
pytestmark = [
    pytest.mark.asyncio,
    pytest.mark.skipif(
        os.getenv("RUN_AGENTBRICKS_GENIE_TESTS") != "1",
        reason="Set RUN_AGENTBRICKS_GENIE_TESTS=1 and AGENTBRICKS_GENIE_SPACE_ID to run live tests.",
    ),
]


def _save(name, value):
    directory = os.getenv("AGENTBRICKS_GENIE_EVIDENCE_DIR")
    if directory:
...
def project(request, tmp_path, monkeypatch):
    ...
    commands = [
        ...
        [
            ...
            "add",
            "genie-agent",
            os.environ["AGENTBRICKS_GENIE_SPACE_ID"],
            "--source",
            str(source),
        ],
    ]
    for command in commands:
        result = runner.invoke(agentbricks, ["-o", "json", *command])
        assert result.exit_code == 0, result.output
    monkeypatch.setenv("AGENTBRICKS_PROJECT_ROOT", str(source))
    _save(
...
async def test_genie_agent_conversation_and_rows(project):
    framework = project
    adapter = importlib.import_module(f"databricks_agentkit.{framework}.genie")
    ...
    question = os.getenv(
        "AGENTBRICKS_GENIE_QUESTION", "How many rows are in samples.nyctaxi.trips?"
    )
    ...
    if expected := os.getenv("AGENTBRICKS_GENIE_EXPECTED_VALUE"):
...
async def test_genie_one_ask_poll_and_result(project):
    framework = project
    adapter = importlib.import_module(f"databricks_agentkit.{framework}.mcp")
    question = os.getenv(
        "AGENTBRICKS_GENIE_QUESTION", "How many rows are in samples.nyctaxi.trips?"
    )
