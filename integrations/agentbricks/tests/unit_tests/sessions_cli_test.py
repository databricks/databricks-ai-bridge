"""CLI-level unit tests for `agentbricks sessions` commands (stores / sessions / items).

Exercises each command's handler + render path with a fake low-level client, covering
both text and JSON output modes. Backfills coverage for src/databricks_agentbricks/cli/sessions.py.
"""
...
from databricks_agentbricks.cli.sessions import items, sessions, stores
