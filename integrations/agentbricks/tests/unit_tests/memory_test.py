"""Unit tests for `agentbricks memory` store rendering (timestamp field mapping)."""

from __future__ import annotations

from click.testing import CliRunner

from databricks_agentbricks.cli import memory as memory_mod
from databricks_agentbricks.cli.memory import stores
...
def test_store_create_suggests_binding_the_store():
    ...
    assert result.exit_code == 0, result.output
    assert "agentbricks memory bind demo" in result.output
...
def test_memory_bind_only_edits_agent_toml(tmp_path):
    ...
    assert result.exit_code == 0, result.output
    assert ctx.client().created == []  # bind never provisions — that's `agentbricks deploy`'s job
    ...
    assert "agentbricks memory stores create" in " ".join(result.output.split())
