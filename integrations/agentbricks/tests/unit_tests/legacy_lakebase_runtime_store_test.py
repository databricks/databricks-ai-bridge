...
def test_backend_uses_a_dedicated_per_app_project() -> None:
    selected = runtime_store.backend("agent-bricks-myapp")

    assert selected.project == "agent-bricks-myapp-runtime-store"
    assert selected.branch == "production"
    assert selected.database == "databricks-postgres"
    assert (
        selected.schema
        == "databricks_agentkit_runtime_" + hashlib.sha256(b"agent-bricks-myapp").hexdigest()[:12]
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("another-agent-bricks-app", "databricks_agentkit_runtime_"),
    ],
)
...
def test_get_or_create_backend_creates_missing_project(monkeypatch) -> None:
    ...
    selected = runtime_store.get_or_create_backend("agent-bricks-myapp", "prof")
