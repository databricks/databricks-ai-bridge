...
def test_create_without_store_names_declares_none(tmp_path: pathlib.Path):
    ...
    AgentProject.create(tmp_path, framework="openai", server="agentbricks").write()
...
def test_load_and_create_share_unsupported_selection_validation(
    ...
    values = {"framework": "langgraph", "server": "agentbricks", selection: "unsupported"}
...
def test_required_project_selections_name_agent_manifest(
    ...
    values = {"framework": '"langgraph"', "server": '"agentbricks"', selection: raw}
    _write_manifest(
        tmp_path,
        "schema_version = 1\n\n[agent]\n"
