...
def response():
    return {
        "name": "runtime-stores/agent-bricks-app",
        "owner": {"app": {"name": "agent-bricks-app", "service_principal_id": "sp-123"}},
        "storage_backend": {
            "lakebase": {
...
def test_backend_uses_api_returned_coordinates(response) -> None:
    backend = runtime_store.backend_from_api("agent-bricks-app", "sp-123", response)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {},
        {"app": {}},
        {"app": {"name": "agent-bricks-app"}},
        {"app": {"name": "other-app", "service_principal_id": "sp-123"}},
        {"app": {"name": "agent-bricks-app", "service_principal_id": "other-sp"}},
    ],
)
def test_backend_rejects_missing_or_mismatched_owner(response, owner):
    ...
    with pytest.raises(AgentCliError, match="does not belong to this app identity"):
        runtime_store.backend_from_api("agent-bricks-app", "sp-123", response)
