"""Tests for the runtime store resolvers (databricks_agentkit.runtime.tool_manifest).

The runtime resolves stores from the explicit arg → env only; it never reads ``agent.toml`` (that
file is the CLI's authoring source, which `agentbricks deploy`/`dev` resolve into env). These tests pin
