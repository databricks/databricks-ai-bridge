"""Unit tests for `agentbricks tracing`: configure/disable binding, experiment provisioning, list/get.
...
def test_default_experiment_name_is_per_project_under_shared():
    ...
    assert tracing_mod.default_experiment_name("my-agent") == "/Shared/agentbricks_traces/my-agent"
    ...
    assert (
        ...
        == "/Shared/agentbricks_traces/my-agent-abc123"
    )
...
def test_create_experiment_idempotent_creates_parent_dir_for_nested_path():
    ...
    assert result == tracing_mod.ResolvedTraceExperiment(
        experiment_id="eid-1", tables=tracing_mod.MLflowTraceTables()
    )
    # the intermediate workspace folder is created before the experiment (mlflow won't make it)
    client.ensure_workspace_dir.assert_called_once_with("/Shared/agentbricks_traces")
...
def test_create_experiment_idempotent_returns_uc_tables():
    ...
    assert result == tracing_mod.ResolvedTraceExperiment(
        experiment_id="eid-uc",
        tables=tracing_mod.MLflowTraceTables(
...
def test_bind_sets_experiment_name(tmp_path: pathlib.Path):
    ...
    with (
        ...
        result = CliRunner().invoke(
            ...
            ["--experiment-name", "/Shared/agentbricks_traces/mine", "--source", str(tmp_path)],
            obj=_Ctx(output="json"),
        )
    assert result.exit_code == 0, result.output
    assert json.loads(result.output) == {"experiment_name": "/Shared/agentbricks_traces/mine"}
    assert AgentProject.load(tmp_path).trace_experiment_name == "/Shared/agentbricks_traces/mine"
...
def test_bind_requires_an_experiment(tmp_path: pathlib.Path):
    # bind needs an experiment (name or id), like `agentbricks memory/sessions bind`; no args -> error,
...
def test_bind_by_experiment_id_stores_resolved_name(tmp_path: pathlib.Path):
    ...
    experiment.name = (
        "/Shared/agentbricks_traces/from-id"  # set explicitly (Mock(name=) is special-cased)
    )
    ...
    assert result.exit_code == 0, result.output
    assert json.loads(result.output) == {"experiment_name": "/Shared/agentbricks_traces/from-id"}
    # stored as the resolved NAME, never the id
    assert AgentProject.load(tmp_path).trace_experiment_name == "/Shared/agentbricks_traces/from-id"
...
def test_unbind_removes_the_experiment_binding(tmp_path: pathlib.Path):
    _project(tmp_path, experiment_name="/Shared/agentbricks_traces/x")
    result = CliRunner().invoke(
...
def test_list_by_explicit_experiment_name(tmp_path: pathlib.Path):
    ...
    assert result.exit_code == 0, result.output
    mlflow.get_experiment_by_name.assert_called_once_with("/Shared/agentbricks_traces/mine")
...
def test_list_defaults_to_projects_bound_experiment(tmp_path: pathlib.Path):
    ...
    _project(tmp_path, experiment_name="/Shared/agentbricks_traces/demo")
    mlflow = mock.Mock()
    ...
    assert result.exit_code == 0, result.output
    mlflow.get_experiment_by_name.assert_called_once_with("/Shared/agentbricks_traces/demo")
...
def test_list_reads_local_dev_store_when_not_provisioned(tmp_path: pathlib.Path):
    # No workspace experiment yet, but a local `agentbricks dev` store exists -> list reads the local traces
    # over a short-lived REST server (not by opening the sqlite file), consistent with where `agentbricks dev`
    # traced pre-deploy, and tears the server down after.
    _project(tmp_path, experiment_name="/Shared/agentbricks_traces/demo")
    (tmp_path / ".agentbricks").mkdir()
    (tmp_path / ".agentbricks" / "mlflow.db").write_text("")  # only needs to exist
    mlflow = mock.Mock()
...
# --- local dev tracing server (`agentbricks dev`) ---------------------------------
...
def test_start_local_tracing_server_launches_sqlite_server(tmp_path: pathlib.Path, monkeypatch):
    ...
    assert "sqlite:///" in joined and ".agentbricks/mlflow.db" in joined
    # pinned interpreter (avoids source-building pyarrow on 3.13) + broad mlflow range (uvx cache reuse
    # across agentbricks releases; the server owns the schema and reads go over REST, so no exact-version pin)
