"""Unit tests for ``agentbricks add-sandbox`` source generation."""
...
def _project(
    ...
    *,
    ...
    if framework is not None:
        metadata = project / ".agentbricks" / "project.toml"
...
def test_generated_block_reads_valid_python_template_at_render_time(
    ...
    (template_dir / "sandbox_mcp.py").write_text(
        "# BEGIN: agentbricks add-sandbox\n"
        "_SANDBOX_DOWNSCOPE: dict[str, list[dict[str, str]]] = {}  "
        "# __AGENTBRICKS_SANDBOX_DOWNSCOPE__\n"
        'TEMPLATE_SOURCE = "loaded"\n'
        "# END: agentbricks add-sandbox\n"
    )
...
def test_add_sandbox_reports_missing_agentbricks_project(tmp_path: pathlib.Path):
    result = CliRunner().invoke(
        add_sandbox,
