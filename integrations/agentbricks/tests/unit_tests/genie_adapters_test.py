...
MESSAGE = "c" * 32
ATTACHMENT = "d" * 32
TEMPLATES = pathlib.Path(__file__).parents[2] / "src/databricks_agentbricks/templates"
...
def _tools(framework):
    return importlib.import_module(f"databricks_agentkit.{framework}").genie_tools()
...
def sdk(monkeypatch):
    ...
    from databricks_agentkit.runtime import genie
...
def test_explicit_missing_project_is_not_silently_ignored(framework, tmp_path, monkeypatch):
    monkeypatch.setenv("AGENTBRICKS_PROJECT_ROOT", str(tmp_path))
    with pytest.raises(RuntimeError, match="AGENTBRICKS_PROJECT_ROOT"):
        _tools(framework)
...
async def test_native_genie_uses_binding_auth_with_request_resolver(framework, project, sdk):
    ...
    tools = importlib.import_module(f"databricks_agentkit.{framework}").genie_tools(
...
async def test_native_genie_cancellation_waits_for_inflight_sdk_call(sdk):
    from databricks_agentkit.runtime.genie import GenieAgent
...
def mcp_adapter(framework):
    ...
    return importlib.import_module(f"databricks_agentkit.{framework}.mcp")
...
module = importlib.import_module('databricks_agentkit.{framework}')
assert 'genie_tools' in module.__all__
assert 'databricks_agentkit.{framework}.genie' not in sys.modules
""",
        ],
        capture_output=True,
        text=True,
