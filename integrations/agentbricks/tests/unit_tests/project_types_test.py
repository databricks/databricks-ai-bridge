"""Selection enums retain the lowercase `agentbricks` server and manifest values."""
...
def test_invalid_cross_enum_value_does_not_leak_enum_representation():
    with pytest.raises(AgentCliError) as error:
        parse_framework(AgentServer.AGENTBRICKS)
    assert error.value.message == "Unsupported Agent Bricks framework 'agentbricks'."
