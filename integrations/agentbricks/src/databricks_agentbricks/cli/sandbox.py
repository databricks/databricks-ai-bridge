...
_DOWNSCOPE_PLACEHOLDER = "# __AGENTBRICKS_SANDBOX_DOWNSCOPE__"
...
def _existing_policy(
    ...
    *,
    ...
    if (
        ...
        raise AgentCliError(
            "The existing agentbricks add-sandbox block has invalid markers.",
    ...
    if len(assignments) != 1:
        raise AgentCliError(
            "The existing agentbricks add-sandbox block is incomplete.",
            hint="Repair or remove the generated block in agent/mcps.py, then retry.",
        )
    ...
    try:
        ...
        raise AgentCliError("The existing agentbricks add-sandbox block is incomplete.") from exc
    if ast.dump(actual_tree, include_attributes=False) != ast.dump(
        expected_tree, include_attributes=False
    ):
        raise AgentCliError(
            "The existing agentbricks add-sandbox block does not match the generated security wrapper.",
            hint="Repair or remove the generated block in agent/mcps.py, then retry.",
        )
...
def _configure_langgraph_runtime(source: str) -> str:
    ...
    try:
        ...
        raise AgentCliError(
            f"Could not parse agent/agentbricks/mcp_runtime.py: {exc.msg} (line {exc.lineno})."
        ) from exc
    ...
    if len(imports) != 1 or imports[0].lineno != imports[0].end_lineno:
        raise AgentCliError(
            "agent/agentbricks/mcp_runtime.py must import build_mcp_servers from agent.mcps.",
    ...
    if not configured:
        ...
        if any(alias.asname is not None for alias in imports[0].names):
            raise AgentCliError(
                "agent/agentbricks/mcp_runtime.py must import build_mcp_servers without an alias."
            )
    ...
    if len(client_calls) != 1:
        raise AgentCliError(
            ...
            "agent/agentbricks/mcp_runtime.py.",
            hint="Start from the Agent Bricks LangGraph agent template, then retry.",
        )
...
class _LangGraphSandboxAdapter:
    def apply(
        ...
        target = project / "agent" / "mcps.py"
        runtime = project / "agent" / "agentbricks" / "mcp_runtime.py"
...
def add_sandbox(
    ...
    if (project / "agent.toml").is_file():
        from databricks_agentbricks.cli.tools import add_sandbox_to_manifest
