...
def _with_trace_read_target(
    ...
    restore it - this is a one-shot process. Managed and local reads need no warehouse.
...
def _start_read_server(db: pathlib.Path) -> tuple[subprocess.Popen | None, Optional[str]]:
    ...
    if not _wait_for_server(base_url, server):
        stop_local_tracing_server(server)
        render.diagnostic(
            "warning", "local trace store did not come up", help="see .agentbricks/mlflow-read.log"
        )
        return None, None
...
def start_local_tracing_server(
    ...
    agent venv; it stores traces in ``<source>/.agentbricks/mlflow.db`` and serves the trace UI + REST API on
...
def tracing_get(obj, trace_id, experiment_name, experiment_id, warehouse_id, source) -> None:
    ...
    with _with_trace_read_target(
        ...
        if target.tracking_uri is None:
            ...
            if warehouse:
                # UC-ness can't be known in this fallthrough, so the provided warehouse is exported
                # for the fetch (harmless for a managed experiment). Not restored - one-shot process.
