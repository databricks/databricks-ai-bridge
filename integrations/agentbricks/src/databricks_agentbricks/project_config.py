"""Persistent Agent Bricks project metadata and legacy framework detection."""
...
def write_project_metadata(
    ...
    *,
    ...
    """Write the metadata consumed by template-aware Agent Bricks commands."""
...
def is_custom_server_template(template: str | None) -> bool:
    """Whether ``template`` is one of the Agent Bricks custom HTTP server templates."""
    return template in _CUSTOM_SERVER_TEMPLATES


def uses_custom_server(project: pathlib.Path) -> bool:
    """Whether persisted project metadata selects an Agent Bricks custom server template."""
