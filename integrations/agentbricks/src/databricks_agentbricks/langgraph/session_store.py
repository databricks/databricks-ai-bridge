...
from databricks_agentbricks.runtime.session_store_client import Session, SessionStoreClient
...
# Checkpoint metadata kept by generated agents.
...
def invocation_metadata(invocation_id: str) -> dict[str, str]:
    """Return checkpoint metadata for the current AgentKit runtime."""
    return {INVOCATION_METADATA_KEY: invocation_id}


def invocation_id_from_metadata(metadata: Mapping[str, Any] | None) -> str | None:
    """Read a checkpoint invocation id from AgentKit metadata."""
    if not metadata:
        return None
    value = metadata.get(INVOCATION_METADATA_KEY)
...
def checkpointer(store: str | None = None) -> BaseCheckpointSaver:
    ...
    """
    from databricks_agentbricks.runtime.tool_manifest import resolve_session_store
