...
def test_document_validate_input():
    ...
    doc = databricks_dspy.DatabricksDocument(data="The Earth orbits the Sun.")
    assert doc.data == "The Earth orbits the Sun."

    with pytest.raises(pydantic.ValidationError):
        # Try to create a `dspy.Document` instance with invalid type.
        databricks_dspy.DatabricksDocument(data=123)


def test_document_in_nested_type():
    class Wrapper(pydantic.BaseModel):
        document: databricks_dspy.DatabricksDocument

    doc = databricks_dspy.DatabricksDocument(data="Hello, world!")
