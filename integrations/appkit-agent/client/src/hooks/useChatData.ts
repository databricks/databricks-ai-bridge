...
import type { Chat, ChatMessage, FeedbackMap } from '@/types';
...
async function fetchChatData(url: string): Promise<ChatData | null> {
  ...
  const chatResponse = await fetch(apiUrl(`/${chatId}`));
  ...
  const messagesResponse = await fetch(apiUrl(`/messages/${chatId}`));
  ...
  try {
    const feedbackResponse = await fetch(apiUrl(`/feedback/chat/${chatId}`));
