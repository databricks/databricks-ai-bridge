...
import type { ChatMessage, Feedback, CustomUIDataTypes, ChatTools } from '@/types';
import {
...
export type MessagePart = UIMessagePart<CustomUIDataTypes, ChatTools>;
...
export interface PreviewMessageProps {
  ...
  /**
   * Override rendering of individual message parts.
   * Return a ReactNode to replace the default, or `undefined` to
   * fall through to the built-in renderer.
   */
  renderPart?: (part: MessagePart, index: number) => ReactNode | undefined;
}
      ...
      >
        ...
        >
          ...
          {partSegments?.map((parts, index) => {
            ...
            if (renderPartOverride) {
              const custom = renderPartOverride(part as MessagePart, index);
              if (custom !== undefined) {
