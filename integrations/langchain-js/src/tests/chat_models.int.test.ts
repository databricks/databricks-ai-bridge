...
import { ChatDatabricks } from "../chat_models.js";
...
describe.skipIf(!hasCredentials)("ChatDatabricks Integration Tests", () => {
  ...
  describe("Tool Calling", () => {
    ...
    it("streams with tool calls", async () => {
      ...
      expect(
        finalChunk.message.tool_calls !== undefined || finalChunk.message.tool_call_chunks !== undefined
      ).toBeTruthy();
    });
  });
  ...
  describe("Error Handling", () => {
    it("throws on invalid endpoint", async () => {
      ...
      await expect(
        badModel.invoke([new HumanMessage("Hello")])
      ).rejects.toThrow();
    });
  });
  ...
  describe("UTF-8 Encoding", () => {
    it("handles unicode content correctly", async () => {
      const response = await model.invoke([
        new HumanMessage("Translate 'hello' to Japanese, Chinese, and Korean. Just give the translations."),
      ]);
    ...
    it("handles emoji in input and output", async () => {
      const response = await model.invoke([
        new HumanMessage("Reply with only: 👋🌍"),
      ]);
