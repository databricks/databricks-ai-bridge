...
class DatabricksServer(Server):
    ...
    def setup_auth(self) -> "DatabricksServer":
        """Set up Databricks OAuth authentication."""
        if self.workspace_client is None:
            self.workspace_client = WorkspaceClient()
        
        # Set up Databricks OAuth authentication and store as a regular attribute
        # This will be picked up by model_dump() since we have extra="allow"
        object.__setattr__(self, "auth", DatabricksOAuthClientProvider(self.workspace_client))
...
class DatabricksMultiServerMCPClient(MultiServerMCPClient):
    ...
    Example:
        ```python
        from databricks.sdk import WorkspaceClient
        from databricks_langchain import DatabricksMultiServerMCPClient, DatabricksServer, Server
    ...
    async def get_tools(self, server_name: str | None = None):
        """
        Get tools from MCP servers, applying server-level configurations.
        
        Args:
            server_name: Optional server name to get tools from. If None, gets tools from all servers.
        
        Returns:
            List of LangChain tools with server-level configurations applied.
        """
        import asyncio
        ...
        load_tool_tasks = [
            asyncio.create_task(super().get_tools(server_name=name))
            for name in server_names
        ]
        tools_list = await asyncio.gather(*load_tool_tasks)
        
        # Apply server-level configurations and collect tools
        all_tools = []
