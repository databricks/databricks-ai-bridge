...
def test_databricks_store_endpoint_configures_lakebase(monkeypatch):
    """Test that DatabricksStore with endpoint uses direct endpoint path."""
    mock_conn = MagicMock()
    test_pool = TestConnectionPool(connection_value=mock_conn)
    monkeypatch.setattr(lakebase, "ConnectionPool", test_pool)
...
@pytest.mark.asyncio
async def test_async_databricks_store_endpoint_configures_lakebase(monkeypatch):
    """Test that AsyncDatabricksStore with endpoint uses direct endpoint path."""
    mock_conn = MagicMock()
    test_pool = TestAsyncConnectionPool(connection_value=mock_conn)
    monkeypatch.setattr(lakebase, "AsyncConnectionPool", test_pool)
...
@pytest.mark.asyncio
async def test_async_databricks_store_endpoint_context_manager(monkeypatch):
    """Test endpoint async store context manager opens and closes the pool."""
    mock_conn = MagicMock()
    test_pool = TestAsyncConnectionPool(connection_value=mock_conn)
    monkeypatch.setattr(lakebase, "AsyncConnectionPool", test_pool)
...
def test_databricks_store_parent_configures_lakebase(monkeypatch):
    """Test that DatabricksStore with parent uses autoscaling path."""
    mock_conn = MagicMock()
    test_pool = TestConnectionPool(connection_value=mock_conn)
    monkeypatch.setattr(lakebase, "ConnectionPool", test_pool)
...
@pytest.mark.asyncio
async def test_async_databricks_store_parent_configures_lakebase(monkeypatch):
    """Test that AsyncDatabricksStore with parent uses autoscaling path."""
    mock_conn = MagicMock()
    test_pool = TestAsyncConnectionPool(connection_value=mock_conn)
    monkeypatch.setattr(lakebase, "AsyncConnectionPool", test_pool)
...
@pytest.mark.asyncio
async def test_async_databricks_store_parent_context_manager(monkeypatch):
    """Test parent async store context manager opens and closes the pool."""
    mock_conn = MagicMock()
    test_pool = TestAsyncConnectionPool(connection_value=mock_conn)
    monkeypatch.setattr(lakebase, "AsyncConnectionPool", test_pool)
