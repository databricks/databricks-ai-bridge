...
def test_checkpoint_saver_v2_configures_lakebase(monkeypatch):
    """V2 CheckpointSaver creates pool with URI conninfo."""
    test_pool = TestConnectionPool(connection_value="lake-conn")
    monkeypatch.setattr(lakebase, "ConnectionPool", test_pool)
    ...
    assert saver._lakebase._is_v2 is True


@pytest.mark.asyncio
async def test_async_checkpoint_saver_v2_configures_lakebase(monkeypatch):
    """V2 AsyncCheckpointSaver creates pool with URI conninfo."""
    test_pool = TestAsyncConnectionPool(connection_value="async-lake-conn")
    monkeypatch.setattr(lakebase, "AsyncConnectionPool", test_pool)
