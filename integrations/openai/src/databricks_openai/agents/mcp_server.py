...
from agents.mcp import (
    MCPServerStreamableHttp,  # ty:ignore[possibly-missing-import]: agents.mcp is only available > 3.9
...
class McpServer(MCPServerStreamableHttp):
    ...
    def create_streams(
        ...
        timeout_value = self.params.get("timeout", 5)
        if isinstance(timeout_value, timedelta):
            timeout: float = float(timeout_value.total_seconds())
        else:
            timeout = float(timeout_value)

        sse_read_timeout_value = self.params.get("sse_read_timeout", 60 * 5)
        if isinstance(sse_read_timeout_value, timedelta):
            sse_read_timeout: float = float(sse_read_timeout_value.total_seconds())
        else:
            sse_read_timeout = float(sse_read_timeout_value)
