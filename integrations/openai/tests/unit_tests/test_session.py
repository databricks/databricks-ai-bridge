...
class TestAsyncDatabricksSessionValidation:
    ...
    def test_endpoint_plus_project_uses_endpoint(
        self, mock_endpoint_workspace_client, mock_engine, mock_event_listens_for
    ):
        """AsyncDatabricksSession with endpoint + project/branch uses endpoint."""
        with (
            patch(
                "databricks_ai_bridge.lakebase.WorkspaceClient",
            ...
            mock_endpoint_workspace_client.postgres.get_endpoint.assert_called()
            mock_endpoint_workspace_client.postgres.list_endpoints.assert_not_called()

    def test_endpoint_plus_instance_raises_error(self):
        """AsyncDatabricksSession with endpoint + instance_name raises ValueError."""
        from databricks_openai.agents.session import AsyncDatabricksSession
...
def mock_endpoint_workspace_client():
    """Create a mock WorkspaceClient for direct endpoint mode."""
    mock_client = MagicMock()
    mock_client.config.host = "https://test.databricks.com"
...
class TestAsyncDatabricksSessionEndpoint:
    """Tests for AsyncDatabricksSession with direct endpoint."""

    def test_init_endpoint_resolves_host(
        self, mock_endpoint_workspace_client, mock_engine, mock_event_listens_for
    ):
        """Test that initialization with endpoint resolves host via get_endpoint API."""
        with (
            patch(
                "databricks_ai_bridge.lakebase.WorkspaceClient",
                return_value=mock_endpoint_workspace_client,
            ...
            # Verify engine URL uses endpoint host
            call_args = mock_create_engine.call_args
            url = call_args[0][0]
            assert url.host == "endpoint-instance.lakebase.databricks.com"

            # Verify get_endpoint API was called
            mock_endpoint_workspace_client.postgres.get_endpoint.assert_called_once_with(
                name="projects/p/branches/b/endpoints/rw"
            )

    def test_init_endpoint_injects_correct_token(
        self, mock_endpoint_workspace_client, mock_engine
    ):
        """Test that do_connect injects endpoint token."""
        captured_handler = None
        ...
        with (
            ...
            # Verify endpoint token was injected
            assert cparams["password"] == "endpoint-oauth-token"
            mock_endpoint_workspace_client.postgres.generate_database_credential.assert_called()

    def test_endpoint_sessions_share_engine(
        self, mock_endpoint_workspace_client, mock_engine, mock_event_listens_for
    ):
        """Test that endpoint sessions with same endpoint share an engine."""
        with (
            patch(
                "databricks_ai_bridge.lakebase.WorkspaceClient",
                return_value=mock_endpoint_workspace_client,
    ...
    def test_different_endpoints_get_different_engines(
        self, mock_endpoint_workspace_client, mock_event_listens_for
    ):
        """Test that sessions with different endpoints get different engines."""
        engine1 = MagicMock()
        engine1.sync_engine = MagicMock()
        engine2 = MagicMock()
        engine2.sync_engine = MagicMock()
        ...
        with (
            ...
            session1 = AsyncDatabricksSession(
                session_id="session-1",
                endpoint="projects/p/branches/b/endpoints/ep-1",
                workspace_client=mock_endpoint_workspace_client,
            )
            session2 = AsyncDatabricksSession(
                session_id="session-2",
                endpoint="projects/p/branches/b/endpoints/ep-2",
                workspace_client=mock_endpoint_workspace_client,
            )
...
@pytest.fixture
def mock_parent_workspace_client():
    ...
    rw_endpoint.name = "projects/my-project/branches/my-branch/endpoints/rw-ep"
    rw_endpoint.status.endpoint_type = "READ_WRITE"
    rw_endpoint.status.hosts.host = "parent-instance.lakebase.databricks.com"
    mock_client.postgres.list_endpoints.return_value = [rw_endpoint]

    # Mock postgres.generate_database_credential for autoscaling token minting
    mock_credential = MagicMock()
    mock_credential.token = "parent-oauth-token"
    mock_client.postgres.generate_database_credential.return_value = mock_credential

    return mock_client


class TestAsyncDatabricksSessionParent:
    """Tests for AsyncDatabricksSession with parent string."""

    def test_init_parent_resolves_host(
        self, mock_parent_workspace_client, mock_engine, mock_event_listens_for
    ):
        """Test that initialization with parent resolves host via list_endpoints API."""
        with (
            patch(
                "databricks_ai_bridge.lakebase.WorkspaceClient",
                return_value=mock_parent_workspace_client,
            ),
            patch(
                "sqlalchemy.ext.asyncio.create_async_engine",
                return_value=mock_engine,
            ...
            AsyncDatabricksSession(
                session_id="test-session-123",
                parent="projects/my-project/branches/my-branch",
                workspace_client=mock_parent_workspace_client,
            )

            # Verify engine URL uses parent-resolved host
            call_args = mock_create_engine.call_args
            url = call_args[0][0]
            assert url.host == "parent-instance.lakebase.databricks.com"

            # Verify list_endpoints API was called with parent
            mock_parent_workspace_client.postgres.list_endpoints.assert_called_once_with(
                parent="projects/my-project/branches/my-branch"
            )

    def test_init_parent_injects_correct_token(
        self, mock_parent_workspace_client, mock_engine
    ):
        """Test that do_connect injects parent token."""
        captured_handler = None
        ...
        with (
            ...
            # Verify parent token was injected
            assert cparams["password"] == "parent-oauth-token"
            mock_parent_workspace_client.postgres.generate_database_credential.assert_called()

    def test_parent_sessions_share_engine(
        self, mock_parent_workspace_client, mock_engine, mock_event_listens_for
    ):
        """Test that parent sessions with same parent share an engine."""
        with (
            patch(
                "databricks_ai_bridge.lakebase.WorkspaceClient",
                return_value=mock_parent_workspace_client,
    ...
    def test_different_parents_get_different_engines(
        self, mock_parent_workspace_client, mock_event_listens_for
    ):
        """Test that sessions with different parents get different engines."""
        engine1 = MagicMock()
        engine1.sync_engine = MagicMock()
        engine2 = MagicMock()
        engine2.sync_engine = MagicMock()
        ...
        with (
            ...
            session1 = AsyncDatabricksSession(
                session_id="session-1",
                parent="projects/p1/branches/b1",
                workspace_client=mock_parent_workspace_client,
            )
            session2 = AsyncDatabricksSession(
                session_id="session-2",
                parent="projects/p2/branches/b2",
                workspace_client=mock_parent_workspace_client,
            )
...
class TestAsyncDatabricksSessionParentValidation:
    """Validation tests for parent parameter."""

    def test_parent_plus_instance_raises_error(self):
        """AsyncDatabricksSession with parent + instance_name raises ValueError."""
        from databricks_openai.agents.session import AsyncDatabricksSession

        workspace = MagicMock()
        workspace.current_user.me.return_value = MagicMock(user_name="test@databricks.com")

        with pytest.raises(ValueError, match="Cannot provide both 'parent' and 'instance_name'"):
            AsyncDatabricksSession(
                session_id="test-session",
    ...
    def test_parent_plus_project_branch_uses_parent(
        self, mock_parent_workspace_client, mock_engine, mock_event_listens_for
    ):
        """AsyncDatabricksSession with parent + project/branch uses parent."""
        with (
            patch(
                "databricks_ai_bridge.lakebase.WorkspaceClient",
                return_value=mock_parent_workspace_client,
            ...
            # Should use list_endpoints (parent path), not get_endpoint
            mock_parent_workspace_client.postgres.list_endpoints.assert_called()
            mock_parent_workspace_client.postgres.get_endpoint.assert_not_called()

    def test_endpoint_plus_parent_uses_endpoint(
        self, mock_endpoint_workspace_client, mock_engine, mock_event_listens_for
    ):
        """AsyncDatabricksSession with endpoint + parent uses endpoint."""
        with (
            patch(
                "databricks_ai_bridge.lakebase.WorkspaceClient",
                return_value=mock_endpoint_workspace_client,
