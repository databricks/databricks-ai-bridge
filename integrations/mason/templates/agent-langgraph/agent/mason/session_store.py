...
from agent.mason.session_store_client import Session, SessionStoreClient
...
class DatabricksSessionStoreSaver(BaseCheckpointSaver[str]):
    ...
    def __init__(
        ...
        *,
        ...
        self._client = (client or SessionStoreClient(workspace_client)).set_session_store(session_store_name)
    ...
    def put(
        ...
        items: list[dict[str, Any]] = [
            self._channel_data_item(channel, str(version), channel_values.get(channel, _EMPTY_CHANNEL_VALUE))
    ...
    def get_tuple(self, config: dict[str, Any]) -> Optional[CheckpointTuple]:
        ...
        return self._build_tuple(
            event, writes_by_ckpt.get(event["checkpoint_id"], []), channel_data, thread_id, actor_id, checkpoint_ns
        )
        ...
        *,
        ...
        for checkpoint_id in sorted(checkpoints, reverse=True):
            ...
            yield self._build_tuple(
                checkpoints[checkpoint_id], writes_by_ckpt.get(checkpoint_id, []), channel_data, thread_id, actor_id, checkpoint_ns
            )
            count += 1
    ...
    async def alist(
        ...
        *,
        ...
        for item in await _run_sync(lambda: list(self.list(config, filter=filter, before=before, limit=limit))):
            yield item
    ...
    ) -> tuple[dict[str, dict[str, Any]], dict[str, list[dict[str, Any]]], dict[tuple[str, str], Any]]:
    ...
    def _build_tuple(
        ...
        pending_writes = [(w["task_id"], w["channel"], self._deserialize(w["value"])) for w in writes]

        parent_config = None
        if checkpoint_event.get("parent_checkpoint_id"):
            parent_config = _config(thread_id, actor_id, checkpoint_ns, checkpoint_event["parent_checkpoint_id"])
    ...
    def _channel_data_item(self, channel: str, version: str, value: Any) -> dict[str, Any]:
        ...
        return {"event_type": _EVENT_CHANNEL_DATA, "channel": channel, "version": version, "value": stored}
...
def _parse_config(config: dict[str, Any]) -> tuple[str, str, str]:
...
def _config(thread_id: str, actor_id: str, checkpoint_ns: str, checkpoint_id: str) -> dict[str, Any]:
    return {
        "configurable": {
            "thread_id": thread_id,
            "actor_id": actor_id,
...
async def _run_sync(fn, *args):
    ...
    return await loop.run_in_executor(None, fn, *args) if args else await loop.run_in_executor(None, fn)
