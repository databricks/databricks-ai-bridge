...
long-term memory and durable conversation history. It creates `<name>-<token>-memory` and
`<name>-<token>-sessions` (a short random token keeps fresh scaffolds from colliding), records both
names in `agent.toml`, and prints them in the `mason init` output.
