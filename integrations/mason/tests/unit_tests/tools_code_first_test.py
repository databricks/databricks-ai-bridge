...
def test_remote_and_local_python_tools_share_one_id_namespace(tmp_path: pathlib.Path) -> None:
    local_first = _project(tmp_path, framework="langgraph")
    runner = CliRunner()
    added_python = runner.invoke(
        tools,
        ["add", "python", "lookup", "--source", str(local_first)],
        obj=_Ctx(),
    )
    duplicate_remote = runner.invoke(
        tools,
        [
            "add",
            "mcp",
    ...
    assert added_python.exit_code == 0, added_python.output
    assert duplicate_remote.exit_code != 0
    assert "lookup" in duplicate_remote.output
    assert "local Python" in duplicate_remote.output
    assert IntegrationRegistry.load(local_first).integrations == []

    remote_first = _project(tmp_path / "other", framework="langgraph")
    added_remote = runner.invoke(
        tools,
        [
            "add",
            "mcp",
    ...
    duplicate_python = runner.invoke(
        tools,
        ["add", "python", "lookup", "--source", str(remote_first)],
        obj=_Ctx(),
    )

    assert added_remote.exit_code == 0, added_remote.output
    assert duplicate_python.exit_code != 0
    assert "lookup" in duplicate_python.output
    assert "Databricks integration" in duplicate_python.output
    assert not (remote_first / "agent" / "tools" / "lookup.py").exists()
