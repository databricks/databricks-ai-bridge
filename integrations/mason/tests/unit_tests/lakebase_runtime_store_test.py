...
def test_legacy_backend_uses_a_dedicated_per_app_project() -> None:
    backend = runtime_store.legacy_backend("agent-mason-myapp")

    assert backend.project == "agent-mason-myapp-runtime-store"
...
def test_get_or_create_legacy_backend_creates_missing_project(monkeypatch) -> None:
    calls = []
    ...
    backend = runtime_store.get_or_create_legacy_backend("agent-mason-myapp", "prof")
