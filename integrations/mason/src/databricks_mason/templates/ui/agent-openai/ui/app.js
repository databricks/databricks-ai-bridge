...
const AGENT_FRAMEWORK = "OpenAI";

const SESSION_STORAGE_KEY = "databricks-mason-session-id";

function newSessionId() {
  if (typeof crypto.randomUUID === "function") return crypto.randomUUID();
  // randomUUID is secure-context-only: Chrome exposes it on localhost but not http://0.0.0.0 or
  // another LAN hostname. getRandomValues remains available there, so build the same UUID v4 shape.
...
async function loadConfig() {
  try {
    ...
    elements.sessionStoreLabel.textContent = config.session.managed
      ? `${config.session.store} · actor ${config.session.actor} · the browser session ID keys the session transcript.`
      : config.session.history
        ? "Messages load from the in-process session for the current browser session."
