...
compatible `databricks-mason[openai]`, supply the real command in `app.yaml`, load configuration
...
translates native events, calls `await context.emit(event)`, and returns JSON output. AgentApp owns
