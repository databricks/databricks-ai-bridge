...
def _workspace_client(profile: Optional[str]) -> WorkspaceClient:
    ...
    return WorkspaceClient(
        ...
        custom_headers={"X-Databricks-Org-Id": str(client.config.workspace_id)},
    )
...
class AgentApiClient:
    ...
    def ensure_workspace_dir(self, path: str) -> None:
        """Create a workspace directory and its parents if needed."""
    ...
    def get_session_store(self, name: str) -> dict:
        return self._do("GET", f"{_BASE}/{session_store_path(name)}")
    ...
    def update_session_store(
        ...
        mask = ",".join(body.keys())
        return self._do(
            "PATCH", f"{_BASE}/{session_store_path(name)}", query=_query(update_mask=mask), body=body
        )
