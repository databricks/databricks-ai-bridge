"""Semantic color palette for the Mason CLI.
...
# rich Theme so styles are referenced by semantic name (`[mason.error]…`) rather than raw hex,
# keeping the color *rules* in one place.
MASON_THEME = Theme(
    {
        "mason.secondary": SECONDARY,
        "mason.dim": DIM,
        "mason.faint": FAINT,
        "mason.success": GREEN,
        "mason.link": BLUE,
        ...
        "mason.sev.error": f"bold {RED}",
        "mason.sev.warning": f"bold {AMBER}",
        "mason.sev.note": f"bold {BLUE}",
    },
    inherit=True,
)
