...
def require_managed_tool_support(server: str) -> None:
    """Reject managed tool bindings for servers that do not consume them."""
    if server != "custom":
        return
    raise AgentCliError(
        'Managed tool bindings in agent.toml require [agent].server = "mason".',
