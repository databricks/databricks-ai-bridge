...
1. **"Did you mean?" on unknown commands.** Click's default dead-ends with
   ``Error: No such command 'foo'.``. `MasonGroup` turns that into the CLI's diagnostic grammar —
...
3. **Grayed `--help`.** `MasonHelpFormatter` renders section headings and description text in the
   ...
   of equal-weight text. Installed on every command via `MasonContext`.
"""
...
class MasonGroup(_FlushEpilog, click.Group):
...
class MasonHelpFormatter(click.HelpFormatter):
...
class MasonCommand(_FlushEpilog, click.Command):
    """A leaf command whose `EXAMPLES` epilog renders flush-left, like the section headings."""


class MasonContext(click.Context):
    """Context that renders help through `MasonHelpFormatter`."""

    formatter_class = MasonHelpFormatter


def _usage_error_show(self: click.exceptions.UsageError, file=None) -> None:
    """Render a Click usage error (bad/unknown option, missing or extra argument) in the mason
    ...
    keep the message verbatim and add the mason `error:` keyword and an indented `help:` pointer.
    """
...
def apply_group_class(root: click.Group, sections: CommandSections) -> None:
    """Upgrade the whole command tree in place: `MasonGroup` on every group (typo suggestions +
    the intent-grouped root listing) and `MasonContext` on every command (grayed `--help`), and
    route Click's own usage errors through the mason diagnostic grammar.
    ...
    """
    _upgrade(root)
    assert isinstance(root, MasonGroup)  # _upgrade just set root.__class__ = MasonGroup
...
def _upgrade(command: click.Command) -> None:
    # Every command — groups and leaves — renders help through MasonContext, and every epilog
    # renders flush-left (MasonGroup / MasonCommand both mix in _FlushEpilog).
    command.context_class = MasonContext
    if isinstance(command, click.Group):
        if not isinstance(command, MasonGroup):
            command.__class__ = MasonGroup
        for child in command.commands.values():
            _upgrade(child)
    elif not isinstance(command, MasonCommand):
        command.__class__ = MasonCommand
