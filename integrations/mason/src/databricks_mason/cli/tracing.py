...
# intentionally NOT restored: ``mason tracing list`` and ``mason tracing get`` are one-shot CLI
...
def _require_warehouse_for_uc_read(experiment, warehouse_id: Optional[str]) -> None:
    ...
    store. The export is NOT restored - ``mason tracing list`` / ``get`` are one-shot processes that
