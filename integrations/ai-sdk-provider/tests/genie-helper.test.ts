...
describe('createDatabricksGenieConversationClient', () => {
  ...
  it('orders attachments using the library display policy and heuristically detects follow-up text', async () => {
    const helper = createHelper(
      createMockFetch([
        {
          method: 'POST',
          ...
          handler: () => ({
            ...
            message: {
              ...
              attachments: [
                {
                  attachment_id: 'attachment-follow-up',
                  text: {
                    content:
                      'Would you like to see the total revenue broken down by pickup and dropoff zip codes instead of the combined route identifier?',
                  },
                },
                {
                  attachment_id: 'attachment-suggested',
                  suggested_questions: {
                    questions: ['Show the top 10 routes by revenue'],
                  },
                },
                {
                  attachment_id: 'attachment-answer',
                  text: {
                    content: 'The total revenue generated by each route is shown below.',
                  },
                },
                {
                  attachment_id: 'attachment-query',
                  query: {
                    description: 'Revenue by route',
                    query: 'SELECT route, total_revenue FROM route_revenue',
                  },
                },
              ],
            },
        ...
        {
          method: 'GET',
          path: '/api/2.0/genie/spaces/space-123/conversations/conversation-6/messages/message-7',
          handler: () => ({
            id: 'message-7',
            conversation_id: 'conversation-6',
            status: 'COMPLETED',
    ...
    const result = await helper.ask('Show revenue by route')

    expect(result.text).toBe('The total revenue generated by each route is shown below.')
    expect(result.attachments.map((attachment) => attachment.type)).toEqual([
      'text',
      'query',
      'text',
    ...
    expect(result.attachments[0]?.attachmentId).toBe('attachment-answer')
    expect(result.attachments[1]?.attachmentId).toBe('attachment-query')
    expect(result.attachments[2]?.attachmentId).toBe('attachment-follow-up')
    expect(result.attachments[3]?.attachmentId).toBe('attachment-suggested')
  })
  ...
  it('supports full query result download helpers', async () => {
    ...
    expect(result.queryResult.statementId).toBe('statement-30')
    expect(result.queryResult.statementState).toBe('SUCCEEDED')
    expect(result.queryResult.externalLinks).toEqual([])
    expect(result.queryResult.rows).toEqual([
      ['A-B', 12.5],
      ['C-D', 10.0],
    ])
  ...
  it('downloads full query results through external links without exposing provider auth headers', async () => {
    const fetch = vi.fn(async (input: string | URL | Request, init?: RequestInit) => {
      ...
      if (
        request.method === 'POST' &&
        url.pathname ===
          '/api/2.0/genie/spaces/space-123/conversations/conversation-31/messages/message-31/attachments/attachment-31/downloads'
      ) {
        return new Response(
          JSON.stringify({
            download_id: 'download-31',
            download_id_signature: 'signature-31',
          }),
          {
            status: 200,
            headers: { 'Content-Type': 'application/json' },
      ...
      if (
        request.method === 'GET' &&
        url.pathname ===
          '/api/2.0/genie/spaces/space-123/conversations/conversation-31/messages/message-31/attachments/attachment-31/downloads/download-31'
      ) {
        return new Response(
          JSON.stringify({
            statement_response: {
              statement_id: 'statement-31',
              status: {
                state: 'SUCCEEDED',
              },
              manifest: {
              ...
              result: {
                external_links: [
                  {
                    external_link: 'https://downloads.example.com/genie-query.csv',
                    http_headers: {
                      'x-databricks-download-token': 'temporary-secret',
                    },
                    row_count: 2000,
                  },
                ],
      ...
      if (
        request.method === 'GET' &&
        url.pathname === '/genie-query.csv'
      ) {
        expect(request.headers.get('authorization')).toBeNull()
        expect(request.headers.get('x-databricks-download-token')).toBe('temporary-secret')

        return new Response('route,revenue\nA-B,12.5\n', {
          status: 200,
    ...
    const helper = createHelper(fetch)
    const result = await helper.downloadQueryResult({
      attachmentId: 'attachment-31',
      conversationId: 'conversation-31',
      messageId: 'message-31',
    })

    expect(result.content).toBe('route,revenue\nA-B,12.5\n')
    expect(result.contentType).toBe('text/csv; charset=utf-8')
    expect(result.fileName).toBe('genie-query-result-attachment-31.csv')
    expect(result.queryResult.externalLinks).toHaveLength(1)
  })

  it('polls refreshed query results until the rerun succeeds', async () => {
    let getQueryResultCount = 0

    const helper = createHelper(
      createMockFetch([
        {
          method: 'POST',
          path: '/api/2.0/genie/spaces/space-123/conversations/conversation-32/messages/message-32/attachments/attachment-32/execute-query',
          handler: () => ({
            statement_response: {
              statement_id: 'statement-32',
              status: {
                state: 'PENDING',
              },
              manifest: {
        ...
        {
          method: 'GET',
          path: '/api/2.0/genie/spaces/space-123/conversations/conversation-32/messages/message-32/attachments/attachment-32/query-result',
          handler: () => {
            getQueryResultCount += 1

            if (getQueryResultCount === 1) {
              return {
                statement_response: {
                  statement_id: 'statement-32',
                  status: {
    ...
    const result = await helper.refreshQueryAttachment({
      attachmentId: 'attachment-32',
      conversationId: 'conversation-32',
      messageId: 'message-32',
    })

    expect(getQueryResultCount).toBe(2)
    expect(result.statementState).toBe('SUCCEEDED')
  ...
  it('streams progressive Genie status, attachments, query results, and the final result', async () => {
    ...
    expect(events.filter((event) => event.type === 'query-result')).toHaveLength(1)
    expect(events.find((event) => event.type === 'query-result')).toMatchObject({
      type: 'query-result',
      attachmentId: 'attachment-40',
      queryResult: {
        statementId: 'statement-40',
