  ...
  try {
    ...
    if (parsed && typeof parsed === 'object' && 'id' in parsed && 'name' in parsed) {
      return {
        ...
        arguments: (parsed as Record<string, unknown>).arguments,
      }
    }
  } catch {
    // Invalid JSON or malformed tool call tag
  }
  return null
}
...
export function parseTaggedToolResult(text: string): ParsedToolResult | null {
  const inner =
    stripEnclosingTag(text, TAGS.LEGACY_RESULT_OPEN, TAGS.LEGACY_RESULT_CLOSE) ??
  ...
  try {
    ...
    // Invalid JSON or malformed tool result tag
  }
  return null
}
...
function stripEnclosingTag(text: string, open: string, close: string): string | null {
