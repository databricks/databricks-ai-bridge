...
class VectorSearchRetrieverToolMixin(BaseModel):
    ...
    def _get_resources(self, index_name: str, embedding_endpoint: str, index_details: IndexDetails) -> List[Resource]:
        resources = []
        if index_name:
        ...
        if (
            index_details.is_databricks_managed_embeddings and
            (managed_embedding := index_details.embedding_source_column.get("embedding_model_endpoint_name", None))
        ):
